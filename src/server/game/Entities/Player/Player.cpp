...
float Player::DodgeDiminishingReturn(float dodge)
{
    ...
    if (dodge > 0.0f) dimdodge = (dodge * Dodge_Cap[pclass-1] /(dodge + (Dodge_Cap[pclass-1])));
    return dimdodge;
}
...
float Player::ParryDiminishingReturn(float parry)
{
    ...
    if (parry > 0.0f) dimparry = (parry * Parry_Cap[pclass-1] /(parry + (Parry_Cap[pclass-1]*k[pclass-1])));
    return dimparry;
}
